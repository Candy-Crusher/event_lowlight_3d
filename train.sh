CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 --master_port=29605 launch.py  --mode=train \
    --train_dataset="12_194 @ MVSEC(dset='train', z_far=100, dataset_location='data/MVSEC/processed_rect_odem/outdoor_day/outdoor_day2', S=2, aug_crop=16, resolution=[(512, 288), (512, 384), (512, 336)], transform=ColorJitter, strides=[1,2,3,4,5,6,7,8,9], dist_type='linear_1_2', aug_focal=0.9)"   \
    ...
    --lr=5e-5 --min_lr=1e-06 --warmup_epochs=3 --epochs=50 --batch_size=1 --accum_iter=2  \
    --save_freq=3 --keep_freq=5 --eval_freq=1  \
    --output_dir="/mnt/sdc/xswu/3d/code/results/ours_finetune_wE_v3" \
    --wandb \
    --use_event_control \

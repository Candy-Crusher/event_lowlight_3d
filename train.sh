CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=29604 launch.py  --mode=train \
    ...
    --train_criterion="CombinedLoss(ConfLoss(Regr3D(L21, norm_mode='avg_dis'), alpha=0.2), enhancement_loss_weight=0, w_exp=1.0, w_col=0.5, w_tvI=20.0, w_spa=1.0, w_tvR=0.5)"  \
    ...
    --output_dir="results/enhancement_snr_raw_mvsec_freezeencoder" \
    --wandb \
    --use_event_control \
